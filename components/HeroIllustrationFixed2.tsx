...
export default function HeroIllustration({ className = "" }: HeroIllustrationProps) {
  ...
  // Evitar hidratación mismatch mostrando versión estática primero
  if (!isMounted) {
    return (
  ...
  return (
    <div className={`w-full flex justify-center ${className}`}>
      ...
      >
        ...
        {/* Animaciones adicionales solo en cliente */}
        {!shouldReduceMotion && (
          <>
            {/* Líneas de subrayado animadas */}
            <motion.rect
              x="80"
              y="193"
              width="400"
